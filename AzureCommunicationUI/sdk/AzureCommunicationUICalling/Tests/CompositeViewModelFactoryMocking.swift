...
struct CompositeViewModelFactoryMocking: CompositeViewModelFactoryProtocol {
  ...
  var createMockParticipantGridCellViewModel:
    ((ParticipantInfoModel) -> ParticipantGridCellViewModel?)?
  var createParticipantsListCellViewModel:
    ((ParticipantInfoModel) -> ParticipantsListCellViewModel?)?
  var createIconButtonViewModel: ((CompositeIcon) -> IconButtonViewModel?)?

  var createCameraIconWithLabelButtonViewModel:
    ((CameraButtonState) -> IconWithLabelButtonViewModel<CameraButtonState>?)?
  var createMicIconWithLabelButtonViewModel:
    ((MicButtonState) -> IconWithLabelButtonViewModel<MicButtonState>?)?
  var createAudioIconWithLabelButtonViewModel:
    ((AudioButtonState) -> IconWithLabelButtonViewModel<AudioButtonState>?)?
  ...
  func makeIconButtonViewModel(
    ...
    return createIconButtonViewModel?(iconName)
      ?? IconButtonViewModel(
        iconName: iconName,
  ...
  func makeIconWithLabelButtonViewModel<ButtonStateType>(
    ...
    isDisabled: Bool,
    action: @escaping (() -> Void)
  ) -> IconWithLabelButtonViewModel<ButtonStateType> where ButtonStateType: ButtonState {
  ...
  func makeAudioDevicesListViewModel(
    dispatchAction: @escaping ActionDispatch,
    localUserState: LocalUserState
  ) -> AudioDevicesListViewModel {
    return audioDevicesListViewModel
      ?? AudioDevicesListViewModel(
        compositeViewModelFactory: self,
        dispatchAction: dispatchAction,
        localUserState: localUserState,
  ...
  func makeSelectableDrawerListItemViewModel(
    ...
    return audioDevicesListCellViewModel
      ?? SelectableDrawerListItemViewModel(
        icon: icon,
        title: title,
  ...
  func makeParticipantCellViewModel(
    participantModel: ParticipantInfoModel, lifeCycleState: LifeCycleState
  ) -> ParticipantGridCellViewModel {
    return createMockParticipantGridCellViewModel?(participantModel)
      ?? ParticipantGridCellViewModel(
        localizationProvider: localizationProvider,
        accessibilityProvider: accessibilityProvider,
  ...
  func makeParticipantsListCellViewModel(participantInfoModel: ParticipantInfoModel)
    -> ParticipantsListCellViewModel
  {
    createParticipantsListCellViewModel?(participantInfoModel)
      ?? ParticipantsListCellViewModel(
  ...
  func makeMoreCallOptionsListViewModel(
    showSharingViewAction: @escaping () -> Void, showSupportFormAction: @escaping () -> Void
  ...
  func makeDrawerListItemViewModel(
    ...
    moreCallOptionsListCellViewModel
      ?? DrawerListItemViewModel(
        icon: icon,
        title: title,
  ...
  func makeLobbyWaitingHeaderViewModel(
    localUserState: LocalUserState,
    dispatchAction: @escaping ActionDispatch
  ) -> LobbyWaitingHeaderViewModel {
    return lobbyWaitingHeaderViewModel
      ?? LobbyWaitingHeaderViewModel(
        compositeViewModelFactory: self,
        logger: logger,
        localUserState: localUserState,
  ...
  func makeLobbyActionErrorViewModel(
    localUserState: AzureCommunicationUICalling.LocalUserState,
    dispatchAction: @escaping AzureCommunicationUICalling.ActionDispatch
  )
    -> LobbyErrorHeaderViewModel
  {
    return lobbyErrorHeaderViewModel
      ?? LobbyErrorHeaderViewModel(
        compositeViewModelFactory: self,
        logger: logger,
        localUserState: localUserState,
