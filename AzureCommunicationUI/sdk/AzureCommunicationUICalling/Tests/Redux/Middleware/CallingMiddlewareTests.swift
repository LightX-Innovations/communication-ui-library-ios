...
class CallingMiddlewareTests: XCTestCase {
  ...
  func
    test_callingMiddleware_apply_when_requestMicrophoneOffLocalUserAction_then_handlerRequestMicMuteCalled()
  {
  ...
  func
    test_callingMiddleware_apply_when_requestMicrophoneOnLocalUserAction_then_handlerRequestMicUnmuteCalled()
  {
  ...
  func
    test_callingMiddleware_apply_when_cameraPermissionGranted_then_handlerOnCameraPermissionIsSet()
  {
    let middlewareDispatch = getEmptyCallingMiddlewareFunction()
  ...
  func
    test_callingMiddleware_apply_when_requestCameraPreviewOn_then_handlerRequestCameraPreviewOnCalled()
  {
    let middlewareDispatch = getEmptyCallingMiddlewareFunction()
  ...
  func
    test_callingMiddleware_apply_when_requestCameraOffLocalUserAction_then_handlerRequestCameraOffCalled()
  {
  ...
  func test_callingMiddleware_apply_when_declineAction_then_handlerDeclineCalled() {
    ...
    middlewareDispatch(getEmptyDispatch())(
      .remoteParticipantsAction(.decline(participantId: "id1")))
    wait(for: [expectation], timeout: 1)
  }
...
extension CallingMiddlewareTests {
  ...
  private func getEmptyCallingMiddlewareFunction() -> (@escaping ActionDispatch) -> ActionDispatch {
    let mockMiddleware: Middleware<AppState, AzureCommunicationUICalling.Action> =
      .liveCallingMiddleware(callingMiddlewareHandler: mockMiddlewareHandler)
