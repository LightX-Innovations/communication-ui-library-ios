...
class CallingMiddlewareHandlerTests: XCTestCase {
  ...
  func
    test_callingMiddlewareHandler_requestMicMute_when_returnsError_then_updateMicrophoneStatusIsError()
    async
  {
    let sut = makeSUT()
    let error = getError()
  ...
  func
    test_callingMiddlewareHandler_requestMicUnmute_when_returnsError_then_updateMicrophoneStatusIsError()
    async
  {
    let sut = makeSUT()
    let error = getError()
  ...
  func
    test_callingMiddlewareHandler_requestCameraOn_when_cameraPermissionNotAsked_then_shouldDispatchCameraPermissionRequested()
    async
  {
    let sut = makeSUT()
    func dispatch(action: Action) {
  ...
  func
    test_callingMiddlewareHandler_requestCameraOff_when_permissionNotAsked_then_updateCameraStatusOffUpdate()
    async
  {
    let sut = makeSUT()
    func dispatch(action: Action) {
    ...
    async
  {
    ...
    func dispatch(action: Action) {
      XCTAssertTrue(
        action == Action.localUserAction(.cameraOnSucceeded(videoStreamIdentifier: videoId)))
      switch action {
  ...
  func
    test_callingMiddlewareHandler_requestCameraOff_when_returnsError_then_updateCameraStatusIsError()
    async
  {
    let sut = makeSUT()
    let error = getError()
  ...
  func
    test_callingMiddlewareHandler_requestCameraOn_when_returnsError_then_updateCameraStatusIsError()
    async
  {
    let sut = makeSUT()
    let error = getError()
  ...
  func
    test_callingMiddlewareHandler_requestCameraSwitch_when_noError_then_updateCameraDeviceStatusOnUpdate()
    async
  {
    let sut = makeSUT()
    ...
    func dispatch(action: Action) {
      XCTAssertTrue(
        action == Action.localUserAction(.cameraSwitchSucceeded(cameraDevice: cameraDevice)))
      switch action {
  ...
  func
    test_callingMiddlewareHandler_requestCameraSwitch_when_returnsError_then_updateCameraDeviceStatusIsError()
    async
  {
    let sut = makeSUT()
    let error = getError()
      ...
      XCTAssertTrue(
        action
          == Action.localUserAction(
            .cameraSwitchFailed(
              previousCamera: LocalUserState.CameraDeviceSelectionStatus.front, error: error)))
    }
    mockCallingService.error = error
    await sut.requestCameraSwitch(state: getEmptyState(), dispatch: dispatch).value
  }
  ...
  func test_callingMiddlewareHandler_endCall_when_returnNSError_then_firstUpdateCallError() async {
    ...
    func dispatch(action: Action) {
      if callBackCount == 0 {
        XCTAssertTrue(
          action
            == Action.errorAction(.fatalErrorUpdated(internalError: .callEndFailed, error: error)))
        switch action {
  ...
  func
    test_callingMiddlewareHandler_endCall_when_returnsCompositeError_then_firstUpdateClientError()
    async
  {
    let sut = makeSUT()
    ...
    func dispatch(action: Action) {
      if callBackCount == 0 {
        XCTAssertTrue(
          action == Action.errorAction(.fatalErrorUpdated(internalError: error, error: nil)))
        switch action {
        case let .errorAction(.fatalErrorUpdated(internalError, err)):
    ...
    async
  {
    ...
    func dispatch(action: Action) {
      XCTAssertTrue(
        action
          == Action.errorAction(.fatalErrorUpdated(internalError: .callJoinFailed, error: error)))
      switch action {
      case let .errorAction(.fatalErrorUpdated(internalError, err)):
        let nserror = err as? NSError
        XCTAssertEqual(nserror?.code, errorCode)
  ...
  func
    test_callingMiddlewareHandler_startCall_when_returnsCompositeError_then_updateClientErrorCompositeError()
    async
  {
    let sut = makeSUT()
  ...
  func
    test_callingMiddlewareHandler_setupCall_when_cameraPermissionDenied_then_skipCameraOnTriggered()
    async
  {
    let sut = makeSUT()
  ...
  func
    test_callingMiddlewareHandler_enterBackground_when_callConnected_cameraStatusOn_then_cameraPausedDispatched()
    async
  {
    let sut = makeSUT()
    func dispatch(action: Action) {
    ...
    await sut.enterBackground(
      state: getState(
        callingState: .connected,
        cameraStatus: .on,
        cameraDeviceStatus: .front), dispatch: dispatch
    ).value
  }

  func
    test_callingMiddlewareHandler_enterBackground_when_callConnected_cameraStatusOn_then_stopLocalVideoStreamCalled()
    async
  {
    let sut = makeSUT()
    await sut.enterBackground(
  ...
  func
    test_callingMiddlewareHandler_enterBackground_when_callNotConnected_then_stopLocalVideoStreamNotCalled()
    async
  {
    let sut = makeSUT()
    await sut.enterBackground(
  ...
  func
    test_callingMiddlewareHandler_enterBackground_when_cameraStatusNotOn_then_stopLocalVideoStreamNotCalled()
    async
  {
    let sut = makeSUT()
    await sut.enterBackground(
  ...
  func
    test_callingMiddlewareHandler_enterBackground_when_callConnected_cameraStatusOn_noError_then_updateCameraStatusPauseUpdate()
    async
  {
    let sut = makeSUT()
    func dispatch(action: Action) {
  ...
  func
    test_callingMiddlewareHandler_enterBackground_when_callConnected_cameraStatusOn_returnsError_then_updateCameraStatusIsError()
    async
  {
    let sut = makeSUT()
    let error = getError()
  ...
  func
    test_callingMiddlewareHandler_enterForeground_when_callConnected_cameraStatusPaused_then_startLocalVideoStreamCalled()
    async
  {
    let sut = makeSUT()
  ...
  func
    test_callingMiddlewareHandler_enterForeground_when_notInBackground_callConnected_cameraStatusPaused_then_willNotStartLocalVideoStreamCalled()
    async
  {
    let sut = makeSUT()
    await sut.enterForeground(
  ...
  func
    test_callingMiddlewareHandler_enterForeground_when_callNotStarted_then_startLocalVideoStreamNotCalled()
    async
  {
    let sut = makeSUT()
    await sut.enterForeground(
  ...
  func
    test_callingMiddlewareHandler_enterForeground_when_cameraStatusNotPaused_then_startLocalVideoStreamNotCalled()
    async
  {
    let sut = makeSUT()
    await sut.enterForeground(
  ...
  func
    test_callingMiddlewareHandler_enterForeground_when_callConnected_cameraStatusOn_noError_then_updateCameraStatusOnUpdate()
    async
  {
    let sut = makeSUT()
    let id = "identifier"
  ...
  func
    test_callingMiddlewareHandler_enterForeground_when_callLocalHold_cameraStatusOn_noError_then_updateCameraStatusOnUpdate()
    async
  {
    let sut = makeSUT()
    let id = "identifier"
  ...
  func
    test_callingMiddlewareHandler_enterForeground_when_callConnected_cameraStatusOn_returnsError_then_updateCameraStatusIsError()
    async
  {
    let sut = makeSUT()
    let error = getError()
  ...
  func
    test_callingMiddlewareHandler_onCameraPermissionIsSet_when_callTransmissionLocal_cameraPermissionRequesting_then_updateCameraPreviewOnTriggered()
    async
  {
    let sut = makeSUT()
    func dispatch(action: Action) {
    ...
    await sut.onCameraPermissionIsSet(
      state: getState(
  ...
  func
    test_callingMiddlewareHandler_onCameraPermissionIsSet_when_callTransmissionRemote_cameraPermissionRequesting_then_updateCameraOnTriggered()
    async
  {
    let sut = makeSUT()
    func dispatch(action: Action) {
  ...
  func
    test_callingMiddlewareHandler_onCameraPermissionIsSet_when_callTransmissionRemote_cameraPermissionNotRequesting_then_updateCameraOnTriggered()
    async
  {
    let sut = makeSUT()
    func dispatch(action: Action) {
  ...
  func test_callingMiddlewareHandler_admit_then_admitCalled() async {
    let sut = makeSUT()
    await sut.admitLobbyParticipant(
      state: getState(
        callingState: .connected,
        cameraStatus: .off,
        cameraDeviceStatus: .front,
