...
class ErrorReducerTests: XCTestCase {
  ...
  func
    test_handleErrorReducer_reduce_when_statusErrorAndCallReset_then_returnErrorState_categoryCallState()
  {
    let state = ErrorState()
    let action = Action.errorAction(
  ...
  func
    test_handleErrorReducer_reduce_when_statusErrorCallEvictionAndCallReset_then_returnErrorState_categoryCallState()
  {
    let state = ErrorState()
  ...
  func
    test_handleErrorReducer_reduce_when_statusErrorCallDeniedAndCallReset_then_returnErrorState_categoryCallState()
  {
    let state = ErrorState()
    let action = Action.errorAction(
      .statusErrorAndCallReset(
  ...
  {
    let state = ErrorState()
    let action = Action.localUserAction(
      .cameraOnFailed(error: CallCompositeInternalError.cameraOnFailed))
    let sut = makeSUT()
    let resultState = sut.reduce(state, action)
