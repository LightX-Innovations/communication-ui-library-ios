...
final class CallDiagnosticsReducerTests: XCTestCase {
  ...
  func
    test_callDiagnosticsReducer_reduce_when_a_media_diagnostic_action_is_received_then_stateUpdated()
  {
  ...
  func
    test_callDiagnosticsReducer_reduce_when_a_network_diagnostic_action_is_received_then_stateUpdated()
  {
  ...
  func
    test_callDiagnosticsReducer_reduce_when_a_network_quality_diagnostic_action_is_received_then_stateUpdated()
  {
  ...
  func
    test_callDiagnosticsReducer_reduce_when_a_media_diagnostic_dismiss_action_is_received_then_stateUpdated()
  {
    let model = MediaDiagnosticModel(diagnostic: .speakingWhileMicrophoneIsMuted, value: true)
    let action = Action.callDiagnosticAction(.media(diagnostic: model))
    let sut = makeSUT()
    ...
    let dismissAction = Action.callDiagnosticAction(
      .dismissMedia(diagnostic: .speakingWhileMicrophoneIsMuted))
  ...
  func
    test_callDiagnosticsReducer_reduce_when_a_network_diagnostic_dismiss_action_is_received_then_stateUpdated()
  {
    let model = NetworkDiagnosticModel(diagnostic: .networkUnavailable, value: true)
    let action = Action.callDiagnosticAction(.network(diagnostic: model))
    let sut = makeSUT()
    ...
    let dismissAction = Action.callDiagnosticAction(
      .dismissNetwork(diagnostic: .networkUnavailable))
    let resultDismiss = sut.reduce(result, dismissAction)
  ...
  func
    test_callDiagnosticsReducer_reduce_when_a_network_quality_diagnostic_dismiss_action_is_received_then_stateUpdated()
  {
    let model = NetworkQualityDiagnosticModel(diagnostic: .networkReceiveQuality, value: .bad)
    let action = Action.callDiagnosticAction(.networkQuality(diagnostic: model))
    let sut = makeSUT()
    ...
    let dismissAction = Action.callDiagnosticAction(
      .dismissNetworkQuality(diagnostic: .networkReceiveQuality))
    let resultDismiss = sut.reduce(result, dismissAction)
