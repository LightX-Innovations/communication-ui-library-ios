...
class AppStateReducerTests: XCTestCase {
  ...
  func test_appStateReducer_reducePermissionState_then_permissionReducerCalled_stateUpdated() {
    ...
    let newPermissionState = PermissionState(
      audioPermission: expectedPermissionState, cameraPermission: expectedPermissionState)
    let mockSubReducer: Reducer<PermissionState, PermissionAction> = .mockReducer(
      outputState: newPermissionState)
  ...
  func test_appStateReducer_reduceLocalUserState_then_localUserReducerCalled_stateUpdated() {
    let oldCameraState = LocalUserState.CameraState(
      operation: .off,
      device: .front,
      transmission: .local)
    let oldAudioState = LocalUserState.AudioState(
      operation: .off,
      device: .receiverSelected)
    let oldLocalUserState = LocalUserState(
    ...
    let expectedCameraState = LocalUserState.CameraState(
      operation: expectedCameraStatus,
      device: expectedCameraDeviceStatus,
      transmission: .local)
    let expectedAudioState = LocalUserState.AudioState(
      operation: expectedMicStatus,
      device: .receiverSelected)

    let newLocalUserState = LocalUserState(
      cameraState: expectedCameraState,
      audioState: expectedAudioState,
      displayName: expectedDisplayName,
      localVideoStreamIdentifier: expectedVideoStreamId)

    let mockSubReducer: Reducer<LocalUserState, LocalUserAction> = .mockReducer(
      outputState: newLocalUserState)
  ...
  func test_appStateReducer_reduceLifeCycleState_then_lifeCycleReducerCalled_stateUpdated() {
    ...
    let mockSubReducer: Reducer<LifeCycleState, LifecycleAction> = .mockReducer(
      outputState: newLifeCycleState)
  ...
  func test_appStateReducer_reduceDiagnosticState_then_diagnosticsStateCalled_stateUpdated() {
    ...
    let mockSubReducer: Reducer<CallDiagnosticsState, Action> = .mockReducer(
      outputState: expectedState)
