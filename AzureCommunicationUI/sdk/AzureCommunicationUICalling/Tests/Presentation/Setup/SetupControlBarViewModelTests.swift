...
class SetupControlBarViewModelTests: XCTestCase {
  ...
  func test_setupControlBarViewModel_when_videoButtonTapped_then_requestCameraOnIfPreviouslyOff() {
    ...
    let cameraState = LocalUserState.CameraState(
      ...
      transmission: .local)
    let sut = makeSUT()
    storeFactory.store.state = AppState(
      permissionState: PermissionState(cameraPermission: .granted),
    ...
    storeFactory.store.$state
      ...
      .sink { [weak self] _ in
        ...
        XCTAssertTrue(
          self?.storeFactory.actions.last == Action.localUserAction(.cameraPreviewOnTriggered))
  ...
  func test_setupControlBarViewModel_when_videoButtonTapped_then_requestCameraOffIfPreviouslyOn() {
    ...
    storeFactory.store.$state
      ...
      .sink { [weak self] _ in
        XCTAssertEqual(self?.storeFactory.actions.count, 1)
        XCTAssertTrue(
          self?.storeFactory.actions.last == Action.localUserAction(.cameraOffTriggered))
  ...
  func test_setupControlBarViewModel_when_micButtonTapped_then_requestMicPreviewOnIfPreviouslyOff()
  {
    ...
    storeFactory.store.$state
      ...
      .sink { [weak self] _ in
        XCTAssertEqual(self?.storeFactory.actions.count, 1)
        XCTAssertTrue(
          self?.storeFactory.actions.last == Action.localUserAction(.microphonePreviewOn))
  ...
  func test_setupControlBarViewModel_when_micButtonTapped_then_requestMicPreviewOffIfPreviouslyOn()
  {
    ...
    storeFactory.store.$state
      ...
      .sink { [weak self] _ in
        XCTAssertEqual(self?.storeFactory.actions.count, 1)
        XCTAssertTrue(
          self?.storeFactory.actions.last == Action.localUserAction(.microphonePreviewOff))
  ...
  func
    test_setupControlBarViewModel_when_updateJoinRequestedTrue_then_buttonViewModelsUpdateDisabled()
  {
    let expectation = XCTestExpectation(
      description: "CameraButtonViewModel disabled state is updated")
    storeFactory.store.state = AppState(
      permissionState: PermissionState(
        audioPermission: .granted,
        cameraPermission: .granted),
  ...
  func
    test_setupControlBarViewModel_when_updateJoinRequestedFalse_AudioAndVideoAreDenied_then_audioAndVideoAreDisabled()
  {
    let sut = makeSUT()
    sut.update(
      localUserState: LocalUserState(),
  ...
  func
    test_setupControlBarViewModel_when_updateJoinRequestedFalse_AudioAndVideoAreGranted_then_audioAndVideoAreDisabled()
  {
    let sut = makeSUT()
    sut.update(
      localUserState: LocalUserState(),
  ...
  func
    test_setupControlBarViewModel_updateStates_when_stateUpdated_then_cameraButtonViewModelButtonInfoUpdated()
  {
  ...
  func
    test_setupControlBarViewModel_updateStates_when_stateUpdated_then_cameraButtonViewModelDisabledStateUpdated()
  {
    let expectation = XCTestExpectation(
      description: "CameraButtonViewModel disabled state is updated")
  ...
  func
    test_setupControlBarViewModel_updateStates_when_stateUpdated_then_micButtonViewModelButtonInfoUpdated()
  {
  ...
  func
    test_setupControlBarViewModel_updateStates_when_stateUpdated_then_audioDeviceButtonViewModelButtonInfoUpdated()
  {
    let expectation = XCTestExpectation(
      description: "AudioDeviceButtonViewModel button info is updated")
...
extension SetupControlBarViewModelTests {
  func makeSUT(
    ...
    return SetupControlBarViewModel(
      compositeViewModelFactory: factoryMocking,
      logger: logger,
      dispatchAction: storeFactory.store.dispatch,
      localUserState: LocalUserState(),
