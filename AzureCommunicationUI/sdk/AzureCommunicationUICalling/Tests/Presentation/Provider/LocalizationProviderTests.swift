...
class LocalizationProviderTests: XCTestCase {
  ...
  func test_localizationProvider_applyRTL_when_layoutDirectionRightToLeft_then_shouldRTLReturnTrue()
  {
    ...
    let localeConfig = LocalizationOptions(
      locale: locale,
  ...
  func
    test_localizationProvider_applyRTL_when_layoutDirectionLeftToRight_then_shouldRTLReturnFalse()
  {
    let sut = makeSUT()
    let locale: Locale = SupportedLocale.en
  ...
  func
    test_localizationProvider_getLocalizedString_when_applyLocaleFr_then_shouldReturnLocalePredefinedString()
  {
    let sut = makeSUT()
