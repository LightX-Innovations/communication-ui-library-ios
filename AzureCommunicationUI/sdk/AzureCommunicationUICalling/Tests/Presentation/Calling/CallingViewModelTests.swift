...
class CallingViewModelTests: XCTestCase {
  ...
  func
    test_callingViewModel_update_when_callStatusIsConnected_then_isLobbyOverlayDisplayed_shouldBecomeFalse()
  {
    let sut = makeSUT()
  ...
  func
    test_callingViewModel_update_when_lifeCycleStateIsBackground_callStatusIsInLobby_then_isLobbyOverlayDisplayed_shouldKeepSame()
  {
    let sut = makeSUT()
    let originalState = sut.lobbyOverlayViewModel.isDisplayed
    let appState = AppState(
      callingState: CallingState(status: .inLobby),
  ...
  func
    test_callingViewModel_update_when_callStatusIsConnected_remoteParticipantNotEmpty_then_isParticipantGridDisplayed_shouldBecomeTrue()
  {
    let sut = makeSUT()
    let mockingParticipantInfoModel = ParticipantInfoModelBuilder.get()
    let remoteParticipantState = RemoteParticipantsState(
      participantInfoList: [mockingParticipantInfoModel],
  ...
  func
    test_callingViewModel_update_when_callStatusIsNotConnected_remoteParticipantNotEmpty_then_isParticipantGridDisplayed_shouldBecomeFalse()
  {
    let sut = makeSUT()
    let mockingParticipantInfoModel = ParticipantInfoModelBuilder.get()
    let remoteParticipantState = RemoteParticipantsState(
  ...
  func
    test_callingViewModel_update_when_callStatusIsNotConnected_remoteParticipantEmpty_then_isParticipantGridDisplayed_shouldBecomeFalse()
  {
    let sut = makeSUT()
    let remoteParticipantState = RemoteParticipantsState(
      participantInfoList: [],
  ...
  func test_callingViewModel_receive_when_statusUpdated_then_controlBarViewModelUpdated() {
    ...
    let updateControlBarViewModel: (LocalUserState, PermissionState, VisibilityState) -> Void = {
      userState, permissionState, _ in
    ...
    factoryMocking.controlBarViewModel = ControlBarViewModelMocking(
  ...
  func test_callingViewModel_receive_when_statusUpdated_then_infoHeaderViewModelUpdated() {
    ...
    let updateInfoHeaderViewModel:
      (LocalUserState, RemoteParticipantsState, CallingState, VisibilityState) -> Void = {
        userState, remoteParticipantsState, callingState, _ in
        XCTAssertEqual(appState.localUserState.displayName, userState.displayName)
        XCTAssertEqual(
          appState.remoteParticipantsState.lastUpdateTimeStamp,
  ...
  func test_callingViewModel_receive_when_statusUpdated_then_participantGridViewModelUpdated() {
    ...
    let appState = AppState()
    let updateParticipantGridViewModel:
      (CallingState, RemoteParticipantsState, VisibilityState, LifeCycleState) -> Void = {
        _, remoteParticipantsState, _, _ in
        XCTAssertEqual(
          appState.remoteParticipantsState.lastUpdateTimeStamp,
          remoteParticipantsState.lastUpdateTimeStamp)
        expectation.fulfill()
  ...
  func test_callingViewModel_receive_when_statusUpdated_then_bannerViewModelUpdated() {
    ...
    factoryMocking.bannerViewModel = BannerViewModelMocking(
      compositeViewModelFactory: factoryMocking,
  ...
  func
    test_callingViewModel_receive_when_callingStateStatusIsLocalHold_then_isOnHoldOverlayDisplayed_shouldBeTrue()
  {
    let sut = makeSUT()
  ...
  func
    test_callingViewModel_update_when_callingStateStatusIsConnected_then_isOnHoldOverlayDisplayed_shouldBeFalse()
  {
    let sut = makeSUT()
    let appState = AppState(callingState: CallingState(status: .connected))
    sut.receive(appState)
  ...
  func test_callingViewModel_receive_when_statusUpdated_then_onHoldOverlayViewModelUpdated() {
    ...
    factoryMocking.onHoldOverlayViewModel = OnHoldOverlayViewModelMocking(
      localizationProvider: localizationProvider,
      compositeViewModelFactory: factoryMocking,
      logger: logger,
      accessibilityProvider: accessibilityProvider,
  ...
  func
    test_callingViewModel_update_when_callStatusIsConnected_appStateForeground_then_switchToBackground_shouldBecomeBackground()
  {
    let sut = makeSUT()
    let appState = AppState(
      callingState: CallingState(status: .connected),
  ...
  func
    test_callingViewModel_update_when_callStatusIsConnected_appStateBackground_then_switchToForeground_shouldBecomeForeground()
  {
    let sut = makeSUT()
    let appState = AppState(
      callingState: CallingState(status: .connected),
