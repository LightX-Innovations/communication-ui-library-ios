...
class CallDiagnosticsViewModelTests: XCTestCase {
    ...
    test_receiving_media_diagnostic_update_should_be_showing_message_bar()
  {
    for diagnostic in MessageBarDiagnosticViewModel.handledMediaDiagnostics {
      ...
      XCTAssertNil(sut.currentBottomToastDiagnostic)
      XCTAssertTrue(
        sut.messageBarStack.first(where: { $0.mediaDiagnostic == diagnostic })?.isDisplayed ?? false
      )
      ...
      XCTAssertNil(sut.currentBottomToastDiagnostic)
      XCTAssertFalse(
        sut.messageBarStack.first(where: { $0.mediaDiagnostic == diagnostic })?.isDisplayed ?? true)
  ...
  func test_receiving_media_unhandled_diagnostic_update_should_not_be_displayed() {
    let otherMediaDiagnostics = MediaCallDiagnostic.allCases.filter({
      !BottomToastDiagnosticViewModel.handledMediaDiagnostics.contains($0)
    })
  ...
  func test_receiving_new_event_bottom_toast_should_override_the_previous_presenting() {
    ...
    let badStateSendReceive = NetworkQualityDiagnosticModel(
      diagnostic: .networkReceiveQuality, value: .bad)
    ...
    sut.update(
      diagnosticsState: CallDiagnosticsState(networkQualityDiagnostic: badStateSendReceive))
    XCTAssertEqual(
      sut.currentBottomToastDiagnostic?.networkQualityDiagnostic, .networkReceiveQuality)
  ...
  func test_bottom_toast_diagnostic_should_be_dismissed_after_interval() {
    ...
    XCTAssertEqual(
      sut.currentBottomToastDiagnostic?.mediaDiagnostic, .speakingWhileMicrophoneIsMuted)

    let expectation = expectation(description: "Wait for timed dismiss")
    XCTWaiter().wait(
      for: [expectation],
      timeout: BottomToastDiagnosticViewModel.bottomToastBannerDismissInterval + 1.0)
