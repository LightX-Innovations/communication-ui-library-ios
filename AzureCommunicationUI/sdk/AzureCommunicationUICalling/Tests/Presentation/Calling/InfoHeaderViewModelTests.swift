...
class InfoHeaderViewModelTests: XCTestCase {
  ...
  func
    test_infoHeaderViewModel_update_when_multipleParticipantInfoListCountChanged_then_shouldBePublished()
  {
    let sut = makeSUT()
    let expectation = XCTestExpectation(description: "Should publish infoLabel")
    sut.$infoLabel
  ...
  func
    test_infoHeaderViewModel_update_when_multipleParticipantInfoListCountChanged_then_shouldBePublishedWithoutInLobbyNorDisconnected()
  {
    let sut = makeSUT()
    let expectation = XCTestExpectation(description: "Should publish infoLabel")
    sut.$infoLabel
  ...
  func test_infoHeaderViewModel_update_when_statesUpdated_then_participantsListViewModelUpdated() {
    ...
    let remoteParticipantsStateValue = RemoteParticipantsState(
      participantInfoList: participantList,
      lastUpdateTimeStamp: Date())
    let localUserStateValue = LocalUserState(displayName: "Updated Name")
    let updateStates: ParticipantsListViewModelUpdateStates = {
      localUserState, remoteParticipantsState in
      XCTAssertEqual(localUserState.displayName, localUserStateValue.displayName)
      XCTAssertEqual(
        remoteParticipantsStateValue.participantInfoList,
  ...
  func
    test_infoHeaderViewModel_didChangeVoiceOverStatus_when_notificationReceived_and_infoHeadeHidden_then_infoHeaderShown()
  {
    let accessibilityProvider = AccessibilityProviderMocking()
  ...
  func test_infoHeaderViewModel_display_infoHeaderLabel0Participant_from_LocalizationMocking() {
    ...
    sut.update(
      ...
      callingState: CallingState(),
      visibilityState: VisibilityState(currentStatus: .visible))
    let expectedInfoHeaderlabel0ParticipantKey =
      "AzureCommunicationUICalling.CallingView.InfoHeader.WaitingForOthersToJoin"
  ...
  func test_infoHeaderViewModel_display_infoHeaderLabel2Participant_from_LocalizationMocking() {
    ...
    let expectedInfoHeaderlabel0ParticipantKey =
      "AzureCommunicationUICalling.CallingView.InfoHeader.WaitingForOthersToJoin"
    let expectedInfoHeaderlabelNParticipantKey =
      "AzureCommunicationUICalling.CallingView.InfoHeader.CallWithNPeople"
