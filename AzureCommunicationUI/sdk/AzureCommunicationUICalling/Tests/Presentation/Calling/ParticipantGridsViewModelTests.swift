...
class ParticipantGridViewModelTests: XCTestCase {
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_screenSharing_then_screenSharingviewModelUpdated()
  {
    let uuid1 = UUID().uuidString
    let uuid2 = UUID().uuidString
    let expectedVideoStreamId = "screenshareVideoStream"
    let screenShareInfoModel = ParticipantInfoModelBuilder.get(
      participantIdentifier: uuid1,
      screenShareStreamId: expectedVideoStreamId)
    let infoModel2 = ParticipantInfoModelBuilder.get(participantIdentifier: uuid2)
    let state = RemoteParticipantsState(
      participantInfoList: [screenShareInfoModel, infoModel2],
    ...
    XCTAssertEqual(
      sut.displayedParticipantInfoModelArr.first!.screenShareVideoStreamModel?
        .videoStreamIdentifier, expectedVideoStreamId)
  }

  func
    test_participantGridsViewModel_updateParticipantsState_when_someParticipantsInLobby_then_lobbyNotDisconnecteParticipantsNotDisplaydInGrid()
  {
    let uuid1 = UUID().uuidString
    let uuid2 = UUID().uuidString
    let uuid3 = UUID().uuidString
    ...
    let infoModel1 = ParticipantInfoModelBuilder.get(
      participantIdentifier: uuid1, screenShareStreamId: nil)
    let infoModel2 = ParticipantInfoModelBuilder.get(
      participantIdentifier: uuid2, screenShareStreamId: nil, status: .inLobby)
    let infoModel3 = ParticipantInfoModelBuilder.get(
      participantIdentifier: uuid3, screenShareStreamId: nil, status: .disconnected)

    let state = RemoteParticipantsState(
      participantInfoList: [infoModel1, infoModel2, infoModel3],
      lastUpdateTimeStamp: Date())
    let sut = makeSUT()
    sut.update(
      callingState: CallingState(),
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_participantViewModelStateChanges_then_participantViewModelUpdated()
  {
    ...
    let expectedUpdatedInfoModel = ParticipantInfoModelBuilder.get(
      participantIdentifier: state.participantInfoList.first!.userIdentifier,
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_participantViewModelStateChanges_then_participantViewModelsStaysTheSame()
  {
    let date = Calendar.current.date(
      byAdding: .minute,
      value: -1,
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_newParticipantAdded_then_participantViewModelAddedToList()
  {
    let date = Calendar.current.date(
      byAdding: .minute,
      value: -1,
    ...
    let updatedParticipantInfoList = [
      state.participantInfoList.first!,
    ...
    sut.update(
      ...
      lifeCycleState: LifeCycleState(currentStatus: .foreground))
    guard
      let newParticipant = sut.participantsCellViewModelArr.first(where: {
        $0.participantIdentifier != participant.participantIdentifier
      })
    else {
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_someParticipantsChanged_then_participantViewModelsListUpdated()
  {
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_participantRemoved_then_participantViewModelsUpdatedAndOrdeingNotChanged()
  {
    let participantsInfoListCount = 3
    let state = makeRemoteParticipantState(count: participantsInfoListCount)
    let callingState = CallingState()
    ...
    XCTAssertEqual(
      sut.participantsCellViewModelArr.map { $0.participantIdentifier },
      updatedInfoList.map { $0.userIdentifier })
  }

  // MARK: LastUpdateTimeStamp
  func
    test_participantGridsViewModel_updateParticipantsState_when_viewModelLastUpdateTimeStampDifferent_then_updateRemoteParticipantCellViewModel()
  {
    ...
    let expectedCount = 2
    let currentState = makeRemoteParticipantState(
      count: expectedCount, lastUpdatedTimeStamp: currentDate)
    let callingState = CallingState()
    let sut = makeSUT()
    sut.update(
      callingState: callingState,
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_newParticipantsJoined_then_participantsJoinedAnnouncementPosted()
  {
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_viewModelLastUpdateTimeStampSame_then_noUpdateRemoteParticipantCellViewModel()
  {
    let date = Calendar.current.date(
      byAdding: .minute,
      value: -1,
    ...
    let firstState = makeRemoteParticipantState(
      count: expectedCount, lastUpdatedTimeStamp: date!,
      dominantSpeakersModifiedTimestamp: date!)
    let currentState = makeRemoteParticipantState(
      count: 2, lastUpdatedTimeStamp: date!,
      dominantSpeakersModifiedTimestamp: date!)
    let callingState = CallingState()
    let sut = makeSUT()
    sut.update(
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_newParticipantJoined_then_participantJoinedAnnouncementPosted()
  {
    let expectation = XCTestExpectation(description: "Announcement expection")
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_participantsLeft_then_participantsLeftAnnouncementPosted()
  {
    let expectation = XCTestExpectation(description: "Announcement expection")
    ...
    let updatedState = RemoteParticipantsState(
      participantInfoList: state.participantInfoList.dropLast(2),
      lastUpdateTimeStamp: Date())
    sut.update(
      callingState: callingState,
      remoteParticipantsState: updatedState,
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_participantLeft_then_participantLeftAnnouncementPosted()
  {
    let expectation = XCTestExpectation(description: "Announcement expection")
    let state = makeRemoteParticipantState(count: 1)
    let callingState = CallingState(status: .connected)
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_participantsListChanged_then_participantLeftAndJoinedAnnouncementPosted()
  {
    let expectation = XCTestExpectation(description: "Announcement expection")
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_oneRemoteParticipant_then_gridsCountSingle_onePaticipantsCellViewModel()
  {
    let expectation = XCTestExpectation(description: "subscription expection")
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_twoRemoteParticipant_then_gridsCountTwo_twoPaticipantsCellViewModel()
  {
    let expectation = XCTestExpectation(description: "subscription expection")
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_threeRemoteParticipant_then_gridsCountThree_threePaticipantsCellViewModel()
  {
    let expectation = XCTestExpectation(description: "subscription expection")
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_fourRemoteParticipant_then_gridsCountFour_fourPaticipantsCellViewModel()
  {
    let expectation = XCTestExpectation(description: "subscription expection")
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_fiveRemoteParticipant_then_gridsCountFive_fivePaticipantsCellViewModel()
  {
    let expectation = XCTestExpectation(description: "subscription expection")
  ...
  func
    test_participantGridsViewModel_updateParticipantsState_when_sevenRemoteParticipant_then_gridsCountSix_sixPaticipantsCellViewModel()
  {
    let expectation = XCTestExpectation(description: "subscription expection")
    let expectedGridCount = 6
    let sut = makeSUT()
...
extension ParticipantGridViewModelTests {
  func makeSUT(
    participantGridCellViewUpdateCompletion: ((ParticipantInfoModel, LifeCycleState) -> Void)? = nil
    ...
    factoryMocking.createMockParticipantGridCellViewModel = { infoModel in
      if let completion = participantGridCellViewUpdateCompletion {
        return ParticipantGridCellViewModelMocking(
  ...
  func makeSUT(
    ...
    let storeFactory = StoreFactoryMocking()
    let factoryMocking = CompositeViewModelFactoryMocking(
      logger: LoggerMocking(),
      store: storeFactory.store,
      accessibilityProvider: accessibilityProvider)
    return ParticipantGridViewModel(
