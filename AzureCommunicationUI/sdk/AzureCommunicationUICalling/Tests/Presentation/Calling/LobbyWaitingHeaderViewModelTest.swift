...
class LobbyWaitingHeaderViewModelTests: XCTestCase {
  ...
  typealias ParticipantsListViewModelUpdateStates = (LocalUserState, RemoteParticipantsState) ->
    Void
  ...
  func
    test_infoHeaderViewModel_update_when_participantInfoListHasNoLobbyParticipants_then_shouldNotBeDisplayed()
  {
    let sut = makeSUT()
    let expectation = XCTestExpectation(description: "Should not display")
    expectation.isInverted = true
    ...
    let participantInfoModel: [ParticipantInfoModel] =
      makeParticipants([
        .connected, .connecting, .disconnected, .earlyMedia, .hold, .idle, .ringing,
      ])
    let remoteParticipantsState = RemoteParticipantsState(
      participantInfoList: participantInfoModel, lastUpdateTimeStamp: Date())
  ...
  func test_infoHeaderViewModel_update_when_lobbyParticipantIsRemoved_then_shouldNotBeDisplayed() {
    ...
    var participantInfoModel: [ParticipantInfoModel] =
      makeParticipants([
        .connected, .connecting, .disconnected, .earlyMedia, .hold, .idle, .ringing, .inLobby,
      ])
    let remoteParticipantsState = RemoteParticipantsState(
      participantInfoList: participantInfoModel, lastUpdateTimeStamp: Date())
  ...
  func
    test_infoHeaderViewModel_update_when_participantInfoListHasLobbyParticipants_then_shouldBeDisplayed()
  {
    let sut = makeSUT()
    ...
    sut.update(
      localUserState: LocalUserState(participantRole: .presenter),
      remoteParticipantsState: remoteParticipantsState,
      callingState: CallingState(),
      visibilityState: VisibilityState())
    ...
    sut.update(
      localUserState: LocalUserState(participantRole: .organizer),
      remoteParticipantsState: remoteParticipantsState,
      callingState: CallingState(),
      visibilityState: VisibilityState())
    ...
    sut.update(
      localUserState: LocalUserState(participantRole: .coOrganizer),
      remoteParticipantsState: remoteParticipantsState,
      callingState: CallingState(),
      visibilityState: VisibilityState())
  ...
  func
    test_infoHeaderViewModel_update_when_participantInfoListHasLobbyParticipantsButHasNoPermissionToAdmit_then_shouldNotBeDisplayed()
  {
    let sut = makeSUT()
    let expectation = XCTestExpectation(description: "Should not display")
    expectation.isInverted = true
    ...
    sut.update(
      localUserState: LocalUserState(participantRole: .consumer),
      remoteParticipantsState: remoteParticipantsState,
      callingState: CallingState(),
      visibilityState: VisibilityState())
    ...
    sut.update(
      localUserState: LocalUserState(participantRole: .uninitialized),
      remoteParticipantsState: remoteParticipantsState,
      callingState: CallingState(),
      visibilityState: VisibilityState())
  ...
  func
    test_infoHeaderViewModel_update_when_participantInfoListHasLobbyParticipantsRoleWithNoPermissionToAdmitThenWith_then_shouldBeDisplayed()
  {
    let sut = makeSUT()
    let expectation = XCTestExpectation(description: "Should display")
    sut.$isDisplayed
