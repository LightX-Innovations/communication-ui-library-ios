...
class LocalVideoViewModelTests: XCTestCase {
  ...
  func
    test_localVideoVideModel_toggleCameraSwitch_when_cameraStatusOn_then_shouldRequestCameraOnTriggered()
  {
    let sut = makeSUT()
    ...
    storeFactory.store.$state
      ...
      .sink { [weak self] _ in
        ...
        XCTAssertTrue(
          self?.storeFactory.actions.first == Action.localUserAction(.cameraSwitchTriggered))
        expectation.fulfill()
