...
class CompositeExitManagerTests: XCTestCase {
  ...
  override func setUp() {
    ...
    compositeExitManager = CompositeExitManager(
  ...
  func test_exitManager_when_connecting_callState_then_compositeExitActionCalled() {
    let callState = CallingState(
      status: CallingStatus.connecting,
    ...
    mockStoreFactory.store.$state
      ...
      .sink { [weak self] _ in
        ...
        XCTAssertTrue(
          self.mockStoreFactory.actions.first == Action.callingAction(.callEndRequested))
  ...
  func test_exitManager_when_disconnected_callState_then_callEndRequestedCalled() {
    let callState = CallingState(
      status: CallingStatus.disconnected,
      operationStatus: OperationStatus.callEnded,
      callId: "123",
      isRecordingActive: false,
      isTranscriptionActive: false,
...
extension CompositeExitManagerTests {
  ...
  func getEventsHandler() -> CallComposite.Events {
    ...
    handler.onDismissed = { [weak self] callCompositeExit in
      ...
      XCTAssertEqual(
        callCompositeExit.error?.localizedDescription, self.expectedError?.localizedDescription)
