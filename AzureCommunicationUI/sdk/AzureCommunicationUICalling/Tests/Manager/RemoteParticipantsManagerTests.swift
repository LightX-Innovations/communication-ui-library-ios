...
class RemoteParticipantsManagerTests: XCTestCase {
  ...
  override func setUp() {
    super.setUp()
    remoteParticipantsJoinedExpectation = XCTestExpectation(
      description: "DidRemoteParticipantsJoin event expectation")
    mockStoreFactory = StoreFactoryMocking()
    eventsHandler = CallComposite.Events()
    avatarViewManager = AvatarViewManagerMocking(
  ...
  func
    test_remoteParticipantsManager_receive_when_stateUpdated_and_participantRemoved_then_avatarViewManagerUpdateStorageCalled()
  {
    let storageUpdatedExpectation = XCTestExpectation(
      description: "AvatarViewManager storage update expectation")
  ...
  func
    test_remoteParticipantsManager_receive_when_stateUpdated_and_noHandlerSet_then_didRemoteParticipantsJoinEventNotPosted()
  {
    makeSUT(isParticipantsJoinHandlerSet: false)
    ...
    let state = AppState(
      remoteParticipantsState: RemoteParticipantsState(participantInfoList: [participantInfoModel]))
    mockStoreFactory.setState(state)
    wait(for: [remoteParticipantsJoinedExpectation], timeout: 1)
  }

  func
    test_remoteParticipantsManager_receive_when_stateUpdated_then_didRemoteParticipantsJoinEventPosted()
  {
    makeSUT()
    let participantInfoModel = ParticipantInfoModel(
      displayName: "Participant 1",
  ...
  func
    test_remoteParticipantsManager_receive_when_participantsLastUpdateTimeStampNotChanged_then_didRemoteParticipantsJoinEventNotPosted()
  {
    makeSUT()
    let participantInfoModel = ParticipantInfoModel(
      displayName: "Participant 1",
  ...
  func
    test_remoteParticipantsManager_receive_when_participantsIdsNotChanged_then_didRemoteParticipantsJoinEventIsNotPosted()
  {
    makeSUT()
    let participantInfoModel = ParticipantInfoModel(
      displayName: "Participant 1",
  ...
  func
    test_remoteParticipantsManager_receive_when_participantLeftCall_then_didRemoteParticipantsJoinEventIsNotPosted()
  {
    makeSUT()
    var participantInfoModels: [ParticipantInfoModel] = []
    for i in 0...5 {
      participantInfoModels.append(
