...
class AvatarManagerTests: XCTestCase {
    ...
    test_avatarManager_setRemoteParticipantViewData_when_personeDataSet_then_participantViewDataDataUpdated()
  {
    ...
    sut.set(
      ...
      XCTAssertEqual(
        sut.avatarStorage.value(forKey: participant.userIdentifier)?.avatarImage!, mockImage)
      expectation.fulfill()
    }
  ...
  func
    test_avatarManager_setRemoteParticipantViewData_when_avatarDataSet__and_participantNotOnCall_then_participantNotFoundErrorReturned()
  {
    guard let mockImage = UIImage.make(withColor: .red) else {
      XCTFail("UIImage does not exist")
      return
