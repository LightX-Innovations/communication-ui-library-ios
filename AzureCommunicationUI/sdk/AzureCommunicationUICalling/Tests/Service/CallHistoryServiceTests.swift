...
class CallHistoryServiceTests: XCTestCase {
  ...
  func test_callingHistoryService_whenCallIDChanges_shouldCallinsertOnRepository() {
    ...
    sut.receive(
      AppState(callingState: CallingState(callId: newCallId, callStartDate: callStartDate)))
...
extension CallHistoryServiceTests {
  func makeSUT() -> CallHistoryServiceMocking {
    return CallHistoryServiceMocking(
      store: storeFactory.store, callHistoryRepository: callHistoryRepository)
  }
}
