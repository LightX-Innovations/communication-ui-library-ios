...
class SharingActivityContainerController: UIViewController, DrawerViewControllerProtocol {
  ...
  init(
    viewModel: DebugInfoSharingActivityViewModel,
  ...
  override func viewDidLoad() {
    ...
    let activityController = UIActivityViewController(
      activityItems: [viewModel.getDebugInfo()],
    ...
    activityController.completionWithItemsHandler = { [weak self] activityType, completed, _, _ in
      ...
      guard isActivityControllerCancelled || isDataSuccessfullyShared || isNavigatedToActivity
      else {
        return
      }
