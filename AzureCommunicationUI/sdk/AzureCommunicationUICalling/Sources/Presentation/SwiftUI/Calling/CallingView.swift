...
struct CallingView: View {
  ...
  var topAlertAreaView: some View {
    GeometryReader { geometry in
      ...
      let infoHeaderViewWidth =
        isIpad
        ? min(
          widthWithoutHorizontalPadding,
  ...
  var infoHeaderView: some View {
    InfoHeaderView(
      viewModel: viewModel.infoHeaderViewModel,
      avatarViewManager: avatarManager)
  }

  var lobbyWaitingHeaderView: some View {
    LobbyWaitingHeaderView(
      viewModel: viewModel.lobbyWaitingHeaderViewModel,
      avatarViewManager: avatarManager)
  }

  var lobbyActionErrorView: some View {
    LobbyErrorHeaderView(
      viewModel: viewModel.lobbyActionErrorViewModel,
      avatarViewManager: avatarManager)
  }
  ...
  var participantGridsView: some View {
    ParticipantGridView(
      viewModel: viewModel.participantGridsViewModel,
      avatarViewManager: avatarManager,
...
extension CallingView {
  ...
  private func updateChildViewIfNeededWith(newOrientation: UIDeviceOrientation) {
    guard !viewModel.controlBarViewModel.isAudioDeviceSelectionDisplayed,
      ...
      !viewModel.controlBarViewModel.isShareActivityDisplayed
    else {
      return
    }
