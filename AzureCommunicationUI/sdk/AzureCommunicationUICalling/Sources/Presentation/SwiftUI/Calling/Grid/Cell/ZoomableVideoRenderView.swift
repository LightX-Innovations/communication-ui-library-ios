...
struct ZoomableVideoRenderView: UIViewRepresentable {
  ...
  private enum InitialZoomScaleConstants {
    ...
    static let minScale: CGFloat =
      isSmallScreen ? SmallScreenConstants.minScale : GeneralScreenConstants.minScale
    static let maxScale: CGFloat =
      isSmallScreen ? SmallScreenConstants.maxScale : GeneralScreenConstants.maxScale
  }
  ...
  init(
    videoRendererViewInfo: ParticipantRendererViewInfo,
  ...
  private func addDoubleTapGestureRecognizer(
    ...
    rendererViewManager?.didRenderFirstFrame =
      coordinator.videoStreamRenderer(didRenderFirstFrameWithSize:)
  ...
  class Coordinator: NSObject, UIScrollViewDelegate, UIGestureRecognizerDelegate {
    ...
    func scrollViewDidZoom(_ scrollView: UIScrollView) {
      ...
      let boundedZoomScale =
        min(scrollView.zoomScale, scrollView.maximumZoomScale) * (1 / initialMinZoomScale)

      if boundedZoomScale > 1 {
        let aspectRatioVideoStream =
          self.streamSize != .zero
          ? self.streamSize.width / self.streamSize.height
          : GeneralScreenConstants.defaultAspectRatio
        ...
        let left =
          0.5
          * (newWidth * boundedZoomScale > scrollView.frame.width
    ...
    private func zoom(_ scrollView: UIScrollView, basedOn point: CGPoint, scale: CGFloat) {
      ...
      if scale == scrollView.maximumZoomScale {
        ...
        scrollView.setZoomScale(
          InitialZoomScaleConstants.minScale,
          animated: true)
      }
    }
