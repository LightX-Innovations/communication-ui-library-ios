...
class AudioSessionManager: AudioSessionManagerProtocol {
  ...
  init(
    ...
    store.dispatch(
      action: .localUserAction(.audioDeviceChangeRequested(device: currentAudioDevice)))
    store.$state
  ...
  private func switchAudioDevice(to selectedAudioDevice: AudioDeviceType) {
    ...
    do {
      ...
      store.dispatch(
        action: .localUserAction(.audioDeviceChangeSucceeded(device: selectedAudioDevice)))
    } catch let error {
