...
extension PermissionsManager {
  ...
  fileprivate func getAudioPermissionStatus() -> AppPermission.Status {
    let audioSession = AVAudioSession.sharedInstance().recordPermission
    return audioSession.map

  }

  fileprivate func getVideoPermissionStatus() -> AppPermission.Status {
    let audioSession = AVCaptureDevice.authorizationStatus(for: .video)
    return audioSession.map

  }

  fileprivate func requestAudioPermissions() -> Future<AppPermission.Status, Never> {
  ...
  fileprivate func requestVideoPermissions() -> Future<AppPermission.Status, Never> {
    return Future { promise in
