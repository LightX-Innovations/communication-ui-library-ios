  ...
  DrawerViewControllerProtocol
{
  ...
  private func getDrawerController(from sourceView: UIView) -> DrawerController? {
    ...
    resizeDrawer()
    self.controller?.contentView?.semanticContentAttribute =
      self.isRightToLeft ? .forceRightToLeft : .forceLeftToRight
    return controller
  }
  ...
  private func resizeDrawer(withLayoutUpdate shouldUpdateLayout: Bool = true) {
    ...
    DispatchQueue.main.async { [weak self] in
      ...
      self.controller?.preferredContentSize = CGSize(
        width: Constants.drawerWidth,
        height: drawerHeight)
    }
  }

  private func getDrawerHeight(
    ...
    var drawerHeight: CGFloat = getTotalCellsHeight(
      tableView: tableView, numberOfItems: numberOfItems)
  ...
  private func getTotalCellsHeight(
    tableView: UITableView,
    numberOfItems: Int
  ) -> CGFloat {
