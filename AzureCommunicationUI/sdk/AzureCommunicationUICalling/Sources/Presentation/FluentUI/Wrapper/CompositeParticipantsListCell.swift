...
class CompositeParticipantsListCell: TableViewCell {
  ...
  func setup(
    viewModel: ParticipantsListCellViewModel,
    ...
    setTitleLabelTextColor(
      color: isNameEmpty ? StyleProvider.color.drawerIconDark : StyleProvider.color.onSurface)

    if viewModel.isInLobby {
      setup(
        title: viewModel.getCellDisplayName(with: participantViewData),
        customView: avatar)
    } else {
      let customAccessoryView = getCustomAccessoryView(
