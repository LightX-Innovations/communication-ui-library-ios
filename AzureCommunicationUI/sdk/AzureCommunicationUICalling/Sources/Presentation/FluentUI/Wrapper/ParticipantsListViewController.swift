...
class ParticipantsListViewController: DrawerContainerViewController<ParticipantsListCellViewModel> {
  ...
  private lazy var participantsListTableView: UITableView? = {
    ...
    tableView.register(
      CompositeParticipantsListCell.self,
  ...
  override func shouldUpdateLayout(items updatedItems: [ParticipantsListCellViewModel]) -> Bool {
    return allParticipants != updatedItems
      ...
      || inCallParticipants.count
        != updatedItems.filter { participantsListCellViewModel in
          !participantsListCellViewModel.isInLobby
        }.count
      || lobbyParticipants.count
        != updatedItems.filter { participantsListCellViewModel in
          participantsListCellViewModel.isInLobby
        }.count
  }
...
extension ParticipantsListViewController: UITableViewDataSource, UITableViewDelegate {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    guard indexPath.row < participantsListCellViewModels.count,
      let cell = tableView.dequeueReusableCell(
        ...
        for: indexPath) as? CompositeParticipantsListCell
    else {
    ...
    cell.setup(
      viewModel: participantCellViewModel,
      avatarViewManager: avatarViewManager)
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    let participantViewData = participantsListCellViewModel.getParticipantViewData(
      from: avatarViewManager)
    let avatarParticipantName = participantsListCellViewModel.getParticipantName(
      with: participantViewData)
  ...
  @objc func onAdmitAllTap() {
    confirmAdmitting(
      title: confirmTitleAdmitAll,
      onConfirmed: self.admintAll,
      onDeclined: self.declineAll)
  }

  private func confirmAdmitting(
    title: String,
    ...
    DispatchQueue.main.async {
      ...
      let admitAlert = UIAlertController(
        title: title,
        message: nil,
        preferredStyle: .alert)
      admitAlert.addAction(
        UIAlertAction(title: self.confirmDecline, style: .cancel) { _ in
          onDeclined()
          self.admitAlert = nil
        })
      admitAlert.addAction(
        UIAlertAction(title: self.confirmAdmit, style: .default) { _ in
          onConfirmed()
          self.admitAlert = nil
        })
