...
extension AzureCommunicationCalling.RemoteParticipant {
  func toParticipantInfoModel() -> ParticipantInfoModel {
    ...
    let screenShareVideoStreamModel = videoInfoModels.first(where: {
      $0.mediaStreamType == .screenSharing
    })
  ...
  >? {
    ...
    return CompositeRemoteParticipant(
      ...
      videoStreams: remote.videoStreams
        .map(
          AzureCommunicationCalling.RemoteVideoStream.toCompositeRemoteVideoStream(
            acsRemoteVideoStream:)
        ),
