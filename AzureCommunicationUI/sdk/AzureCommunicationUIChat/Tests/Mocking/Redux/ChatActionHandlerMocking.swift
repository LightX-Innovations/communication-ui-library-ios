...
class ChatActionHandlerMocking: ChatActionHandling {
  ...
  func initialize(
    state: ChatAppState, dispatch: @escaping ActionDispatch,
  ...
  func disconnectChat(dispatch: @escaping AzureCommunicationUIChat.ActionDispatch) -> Task<
    Void, Never
  > {
    Task {
  ...
  func getListOfParticipants(
    ...
    dispatch: @escaping AzureCommunicationUIChat.ActionDispatch
  ) -> Task<Void, Never> {
    Task {
  ...
  func sendMessage(
    internalId: String, content: String, state: ChatAppState, dispatch: @escaping ActionDispatch
  ) -> Task<Void, Never> {
    Task {
  ...
  func editMessage(
    messageId: String, content: String, prevContent: String, state: ChatAppState,
  ...
  func deleteMessage(messageId: String, state: ChatAppState, dispatch: @escaping ActionDispatch)
    -> Task<Void, Never>
  {
    Task {
  ...
  func setTypingParticipantTimer(
    _ getState: @escaping () -> AzureCommunicationUIChat.ChatAppState,
  ...
  func sendReadReceiptSuccess(messageId: String, dispatch: @escaping ActionDispatch) -> Task<
    Void, Never
  > {
    Task {
