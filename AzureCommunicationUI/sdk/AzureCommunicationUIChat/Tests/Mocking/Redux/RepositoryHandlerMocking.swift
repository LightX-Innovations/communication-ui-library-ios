...
class RepositoryHandlerMocking: RepositoryMiddlewareHandling {
  ...
  func loadInitialMessages(
    messages: [ChatMessageInfoModel], state: ChatAppState, dispatch: @escaping ActionDispatch
  ) -> Task<Void, Never> {
    Task {
  ...
  func updateNewEditedMessage(
    messageId: String, content: String, state: ChatAppState, dispatch: @escaping ActionDispatch
  ) -> Task<Void, Never> {
    Task {
  ...
  func updateNewDeletedMessage(
    messageId: String, state: ChatAppState, dispatch: @escaping ActionDispatch
  ) -> Task<Void, Never> {
    Task {
  ...
  func addTopicUpdatedMessage(
    threadInfo: ChatThreadInfoModel, state: ChatAppState, dispatch: @escaping ActionDispatch
  ) -> Task<Void, Never> {
    Task {
  ...
  func participantRemovedMessage(
    participants: [ParticipantInfoModel], dispatch: @escaping ActionDispatch
  ) -> Task<Void, Never> {
    Task {
  ...
  func addReceivedMessage(
    message: ChatMessageInfoModel, state: ChatAppState, dispatch: @escaping ActionDispatch
  ) -> Task<Void, Never> {
    Task {
  ...
  func addLocalUserRemovedMessage(state: ChatAppState, dispatch: @escaping ActionDispatch) -> Task<
    Void, Never
  > {
    Task {
