...
class ErrorReducerTests: XCTestCase {
  ...
  func
    test_handleErrorReducer_reduce_when_statusErrorChatEvictionAndChatReset_then_returnErrorState_categoryChatState()
  {
    let state = ErrorState()
    let action = Action.errorAction(
      .fatalErrorUpdated(
  ...
  func
    test_handleErrorReducer_reduce_when_statusErrorChatInitFailed_then_returnErrorState_categoryChatState()
  {
    let state = ErrorState()
