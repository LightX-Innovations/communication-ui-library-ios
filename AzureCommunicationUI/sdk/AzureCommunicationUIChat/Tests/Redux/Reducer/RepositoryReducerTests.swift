...
class RepositoryReducerTests: XCTestCase {
  ...
  func
    test_repositoryReducer_reduce_when_fetchPreviousMessagesSuccessAction_notEmptyMessages_then_stateUpdated()
  {
    let initialTimestamp = Date()
    let state = RepositoryState(
      hasFetchedInitialMessages: false,
      hasFetchedAllMessages: false)
    let action = Action.repositoryAction(
      .fetchPreviousMessagesSuccess(messages: [
  ...
  func
    test_repositoryReducer_reduce_when_fetchPreviousMessagesSuccessAction_emptyMessages_then_stateUpdated()
  {
    let initialTimestamp = Date()
    let state = RepositoryState(
      hasFetchedInitialMessages: false,
