...
class ParticipantReducerTests: XCTestCase {
  func
    test_participantsReducer_reduce_when_fetchListOfParticipantsSuccessParticipantAction_then_stateUpdated()
  {
    let participants = [
    ...
    let action = Action.participantsAction(
      .fetchListOfParticipantsSuccess(
        participants: participants, localParticipantId: "localParticipantId"))
    let sut = getSUT()
  ...
  func
    test_participantsReducer_reduce_when_fetchListOfParticipantsSuccessParticipantActionWithMaskedParticipants_then_stateUpdated()
  {
    ...
    let participants = [
      ParticipantInfoModel(
        identifier: CommunicationUserIdentifier(localParticipantId),
        displayName: "localParticipantId"),
      ParticipantInfoModel(
        identifier: CommunicationUserIdentifier(maskedParticipantId),
  ...
  func test_participantsReducer_reduce_when_participantsAddedParticipantAction_then_stateUpdated() {
    let initialParticipantsMap = [
      "id1": ParticipantInfoModel(
        identifier: CommunicationUserIdentifier("id1"),
        displayName: "displayName1"),
      "id2": ParticipantInfoModel(
        identifier: CommunicationUserIdentifier("id2"),
        displayName: "displayName2"),
    ]
    let newParticipants = [
      ParticipantInfoModel(
        identifier: CommunicationUserIdentifier("id3"),
        displayName: "displayName3"),
      ParticipantInfoModel(
        identifier: CommunicationUserIdentifier("id4"),
        displayName: "displayName4"),
      ParticipantInfoModel(
        identifier: CommunicationUserIdentifier("id5"),
  ...
  func test_participantsReducer_reduce_when_participantsRemovedParticipantAction_then_stateUpdated()
  {
    let initialParticipantsMap = [
      ...
      "id2": ParticipantInfoModel(
        identifier: CommunicationUserIdentifier("id2"),
        displayName: "displayName2"),
      "id3": ParticipantInfoModel(
        identifier: CommunicationUserIdentifier("id3"),
        displayName: "displayName3"),
    ]
    let initialTypingParticipants: [UserEventTimestampModel] = [
      UserEventTimestampModel(
        userIdentifier: CommunicationUserIdentifier("id1"),
        timestamp: Iso8601Date())!,
      UserEventTimestampModel(
        userIdentifier: CommunicationUserIdentifier("id2"),
        timestamp: Iso8601Date())!,
    ]
    ...
    let readReceiptMap: [String: Date] = [
      "id1": .distantPast, "id2": .distantPast, "id3": .distantPast,
    ]
    let state = ParticipantsState(
      participants: initialParticipantsMap,
      participantsUpdatedTimestamp: initialTimestamp,
  ...
  func
    test_participantReducer_reduce_when_typingIndicatorReceivedAction_then_participantsStateUpdated()
  {
    let expectedIdentifier = "ID"
    let model = UserEventTimestampModel(
      userIdentifier: CommunicationUserIdentifier(expectedIdentifier),
      timestamp: Iso8601Date())!
    let state = ParticipantsState()
  ...
  func
    test_participantReducer_reduce_when_maskedParticipantsReceived_then_participantsStateUpdated()
  {
    let state = ParticipantsState()
    let action = Action.participantsAction(
      .maskedParticipantsReceived(participantIds: ["id1", "id2", "id3"]))
    let sut = getSUT()
    let resultState = sut.reduce(state, action)
  ...
  func test_participantReducer_reduce_when_participantRemovedAction_then_participantsStateUpdated()
  {
    let expectedIdentifier = "ID"
    let initialParticipantsMap = [
      expectedIdentifier: ParticipantInfoModel(
        identifier: CommunicationUserIdentifier(expectedIdentifier),
        displayName: "displayName1")
    ]
    let existingParticipant = UserEventTimestampModel(
      userIdentifier: CommunicationUserIdentifier(expectedIdentifier),
      timestamp: Iso8601Date())!
    let state = ParticipantsState(
      participants: initialParticipantsMap,
      typingParticipants: [existingParticipant])
    let participantToBeRemoved = ParticipantInfoModel(
      identifier: CommunicationUserIdentifier(expectedIdentifier),
      displayName: expectedIdentifier)
    let action = Action.participantsAction(
      .participantsRemoved(participants: [participantToBeRemoved]))
    let sut = getSUT()
    let resultState = sut.reduce(state, action)
    XCTAssertEqual(resultState.typingParticipants.count, 0)
  }
  ...
  func
    test_participantReducer_reduce_when_newTextMessageReceivedAction_then_participantsStateUpdated()
  {
    let expectedIdentifier = "ID"
    let existingParticipant = UserEventTimestampModel(
      userIdentifier: CommunicationUserIdentifier(expectedIdentifier),
  ...
  func
    test_participantReducer_reduce_when_newHTMLMessageReceivedAction_then_participantsStateUpdated()
  {
    let expectedIdentifier = "ID"
    let existingParticipant = UserEventTimestampModel(
      userIdentifier: CommunicationUserIdentifier(expectedIdentifier),
  ...
  func
    test_participantReducer_reduce_when_newCustomMessageReceivedAction_then_participantsStateUpdated()
  {
    let expectedIdentifier = "ID"
    let existingParticipant = UserEventTimestampModel(
      userIdentifier: CommunicationUserIdentifier(expectedIdentifier),
  ...
  func test_participantReducer_reduce_when_readReceiptReceivedAction_then_participantsStateUpdated()
  {
    ...
    let readReceiptInfo = ReadReceiptInfoModel(
      senderIdentifier: CommunicationUserIdentifier(participantId), chatMessageId: messageId,
      readOn: Iso8601Date())
    ...
    XCTAssertEqual(resultState.readReceiptMap.count, 1)
    XCTAssertEqual(
      resultState.readReceiptMap[participantId], messageId.convertEpochStringToTimestamp())
    XCTAssertEqual(
      resultState.readReceiptUpdatedTimestamp, messageId.convertEpochStringToTimestamp())
  }
}
