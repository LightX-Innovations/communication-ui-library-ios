...
class ChatActionHandlerTests: XCTestCase {
  ...
  func test_chatActionHandler_initialize_then_initializeCalled() async {
    ...
    await sut.initialize(
      ...
      serviceListener: mockChatServiceEventHandler
    ).value
  ...
  func
    test_chatActionHandler_getPreviousMessages_when_nonEmptyPreviousMessages_then_getPreviousMessagesCalled()
    async
  {
    ...
    func dispatch(action: Action) {
      ...
      XCTAssertTrue(
        action == Action.repositoryAction(.fetchPreviousMessagesSuccess(messages: previousMessages))
      )
      expectation.fulfill()
    }
  ...
  func
    test_chatActionHandler_getPreviousMessages_when_emptyPreviousMessages_then_getPreviousMessagesCalled()
    async
  {
  ...
  func
    test_chatActionHandler_recieveTypingIndicator_then_dispatchClearIdleTypingParticipantsAction()
    async
  {
    let expectation = XCTestExpectation(
      description: "Dispatch Clear Idle Typing Participants Success")
    let sut = makeSUT()
    func dispatch(action: Action) {
      switch action {
...
extension ChatActionHandlerTests {
  ...
  func makeSUT(
    initialMessages: [ChatMessageInfoModel]? = nil,
    previousMessages: [ChatMessageInfoModel]? = nil
  ) -> ChatActionHandler {
