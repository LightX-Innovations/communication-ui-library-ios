...
class MessageListViewModel: ObservableObject {
  ...
  func fetchMessages(lastSeenMessage: ChatMessageInfoModel) {
    if let lastSeenMessageIndex = messages.firstIndex(where: { $0.id == lastSeenMessage.id }),
      lastSeenMessageIndex <= minFetchIndex && hasFetchedPreviousMessages && !hasFetchedAllMessages
    {
  ...
  func updateReadReceiptToBeSentMessageId(message: ChatMessageInfoModel) {
    ...
    if let messageTimestamp = message.id.convertEpochStringToTimestamp(),
      let toBeSentReadReceiptTimestamp = readReceiptToBeSentMessageId?
        .convertEpochStringToTimestamp(),
  ...
  func startDidEndScrollingTimer() {
    ...
    didEndScrollingTimer = Timer.scheduledTimer(
      ...
      guard let messageId = self?.readReceiptToBeSentMessageId,
        ...
        toBeSentReadReceiptTimestamp > lastSentReadReceiptTimestamp
      else {
        return
      }
  ...
  func updateJumpToNewMessages() {
    ...
    jumpToNewMessagesButtonLabel = getJumpToNewMessagesLabel(
