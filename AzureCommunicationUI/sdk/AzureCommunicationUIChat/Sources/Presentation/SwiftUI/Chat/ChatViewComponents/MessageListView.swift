...
struct MessageListView: View {
  ...
  var previousFetchActivityIndicator: some View {
    Group {
      if !viewModel.hasFetchedPreviousMessages && !viewModel.hasFetchedAllMessages {
        ActivityIndicator(size: .large)
          .isAnimating(true)
          .padding()
      }
  ...
  private func createMessage(
    ...
    let showDateHeader =
      index == 0 || message.createdOn.dayOfYear - lastMessage.createdOn.dayOfYear > 0
    ...
    VStack {
        ...
        let iconName = message.getIconNameForMessageSendStatus()
      {
        StyleProvider.icon.getImage(for: iconName)
          .frame(
            width: Constants.messageSendStatusIconSize,
