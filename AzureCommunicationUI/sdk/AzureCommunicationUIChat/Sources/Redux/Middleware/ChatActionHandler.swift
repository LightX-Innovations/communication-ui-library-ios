...
protocol ChatActionHandling {
  ...
  func initialize(
    ...
    serviceListener: ChatServiceEventHandling
  ) -> Task<Void, Never>
  @discardableResult
  func getInitialMessages(state: ChatAppState, dispatch: @escaping ActionDispatch) -> Task<
    Void, Never
  >
  @discardableResult
  func getListOfParticipants(state: ChatAppState, dispatch: @escaping ActionDispatch) -> Task<
    Void, Never
  >
  @discardableResult
  func getPreviousMessages(state: ChatAppState, dispatch: @escaping ActionDispatch) -> Task<
    Void, Never
  >
  @discardableResult
  func sendMessage(
  ...
  @discardableResult
  func sendTypingIndicator(state: ChatAppState, dispatch: @escaping ActionDispatch) -> Task<
    Void, Never
  >
  @discardableResult
  func sendReadReceipt(messageId: String, state: ChatAppState, dispatch: @escaping ActionDispatch)
    -> Task<Void, Never>
  func setTypingParticipantTimer(
    _ getState: @escaping () -> ChatAppState, _ dispatch: @escaping ActionDispatch)
}
...
class ChatActionHandler: ChatActionHandling {
    ...
    -> Task<Void, Never>
  {
    Task {
      ...
      guard let messageTimestamp = messageId.convertEpochStringToTimestamp(),
        messageTimestamp > lastReadReceiptSentTimestamp
      else {
        return
      }
      await sendReadReceiptToChatService(messageId: messageId, dispatch: dispatch)
  ...
  private func sendReadReceiptToChatService(messageId: String, dispatch: @escaping ActionDispatch)
    async
  {
    do {
  ...
  func setTypingParticipantTimer(
    ...
    scheduleTimer(
      timeInterval: UserEventTimestampModel.typingParticipantTimeout,
      getState: getState,
      dispatch: dispatch)
  }
  ...
  > {
    Task {
      do {
        ...
        dispatch(
          .participantsAction(.maskedParticipantsReceived(participantIds: maskedParticipantIds)))
...
extension ChatActionHandler {
  ...
  private func handleTimerInterval(
    _ timer: Timer,
    _ dispatch: @escaping ActionDispatch,
    _ getState: @escaping () -> ChatAppState
  ) {
    ...
    scheduleTimer(
      timeInterval: expiringInSeconds,
      getState: getState,
      dispatch: dispatch)
  }
}
