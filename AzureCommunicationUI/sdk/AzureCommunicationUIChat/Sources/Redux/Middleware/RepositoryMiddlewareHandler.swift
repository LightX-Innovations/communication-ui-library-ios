...
class RepositoryMiddlewareHandler: RepositoryMiddlewareHandling {
  ...
  func addNewSentMessage(
    ...
    Task {
      guard let localUserId = state.chatState.localUser?.identifier.stringValue,
        let displayName = state.chatState.localUser?.displayName
      else {
        return
      }
  ...
  func updateSentMessageIdAndSendStatus(
    ...
    Task {
      messageRepository.replaceMessageId(
  ...
  func participantAddedMessage(
    ...
    Task {
      let filteredParticipants = filterOutMaskedParticipantsFromMessage(
        participants: participants, state: state)
  ...
  func updateMessageSendStatus(
    ...
    Task {
      messageRepository.updateMessageSendStatus(
        messageId: messageId, messageSendStatus: messageSendStatus)
      dispatch(.repositoryAction(.repositoryUpdated))
    }
  }
  ...
  func updateMessageReceiptReceivedStatus(
    ...
    Task {
      messageRepository.updateMessageReadReceiptStatus(
        readReceiptInfo: readReceiptInfo, state: state)
      dispatch(.repositoryAction(.repositoryUpdated))
    }
  }
