...
extension Middleware where State == ChatAppState {
  ...
  private static func handleChatAction(
    ...
    case .chatTopicUpdated(let threadInfo):
      actionHandler.addTopicUpdatedMessage(
        threadInfo: threadInfo,
        state: getState(),
        dispatch: dispatch)
    case .chatMessageLocalUserRemoved:
      actionHandler.addLocalUserRemovedMessage(
        state: getState(),
        dispatch: dispatch)
    default:
      break
  ...
  private static func handleParticipantsAction(
    ...
    case .participantsAdded(let participants):
      actionHandler.participantAddedMessage(
        participants: participants,
        state: getState(),
        dispatch: dispatch)
    case .participantsRemoved(let participants):
      actionHandler.participantRemovedMessage(
        participants: participants,
        dispatch: dispatch)
  ...
  private static func handleRepositoryAction(
    ...
    case .chatMessageReceived(let message):
      actionHandler.addReceivedMessage(
        message: message,
        state: getState(),
        dispatch: dispatch)
    case .chatMessageEditedReceived(let message):
      actionHandler.updateReceivedEditedMessage(
        message: message,
        state: getState(),
        dispatch: dispatch)
    case .chatMessageDeletedReceived(let message):
      actionHandler.updateReceivedDeletedMessage(
        message: message,
        state: getState(),
        dispatch: dispatch)
    default:
