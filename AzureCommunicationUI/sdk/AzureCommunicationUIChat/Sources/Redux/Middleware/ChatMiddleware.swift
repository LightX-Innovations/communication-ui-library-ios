...
extension Middleware where State == ChatAppState {
  ...
  private static func handleChatAction(
    ...
    case .sendTypingIndicatorTriggered:
      actionHandler.sendTypingIndicator(
        state: getState(),
        dispatch: dispatch)
  ...
  private static func handleRepositoryAction(
    ...
    case .fetchInitialMessagesTriggered:
      actionHandler.getInitialMessages(
        state: getState(),
        dispatch: dispatch)
      actionHandler.getListOfParticipants(
        state: getState(),
        dispatch: dispatch)
    case .fetchPreviousMessagesTriggered:
      actionHandler.getPreviousMessages(
        state: getState(),
        dispatch: dispatch)
