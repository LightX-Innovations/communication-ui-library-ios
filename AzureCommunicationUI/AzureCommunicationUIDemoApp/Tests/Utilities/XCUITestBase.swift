...
extension XCUITestBase {
  ...
  func takeScreenshot(
    name: String = "App Screenshot - \(Date().description)",
  ...
  func closeDemoAppSettingsPage() {
    if #unavailable(iOS 15) {
      ...
      let startPoint = app.navigationBars.firstMatch.coordinate(
  ...
  func enableMockCallingSDKWrapper() {
    tapButton(
      accessibilityIdentifier: AccessibilityId.useMockCallingSDKHandlerToggleAccessibilityID
        .rawValue)
  }
