...
class EntryViewController: UIViewController {
  ...
  private func setupUI() {
    ...
    startSwiftUIButton.contentEdgeInsets = UIEdgeInsets.init(
    ...
    startSwiftUIButton.addTarget(
      self, action: #selector(onCallingSwiftUIPressed), for: .touchUpInside)
    ...
    startChatSwiftUIButton.contentEdgeInsets = UIEdgeInsets.init(
      top: 10, left: 20, bottom: 10, right: 20)
    ...
    startChatSwiftUIButton.addTarget(
      self, action: #selector(onChatSwiftUIPressed), for: .touchUpInside)

    let startChatUIKitButton = UIButton()
    startChatUIKitButton.backgroundColor = .systemBlue
    startChatUIKitButton.contentEdgeInsets = UIEdgeInsets.init(
      top: 10, left: 20, bottom: 10, right: 20)
  ...
  @objc func onCallingSwiftUIPressed() {
    #if DEBUG
      let swiftUIDemoView = CallingDemoView(
        envConfigSubject: envConfigSubject,
        callingViewModel: callingViewModel,
        callingSDKWrapperMock: callingSDKWrapperMock)
    #else
      let swiftUIDemoView = CallingDemoView(
        envConfigSubject: envConfigSubject, callingViewModel: callingViewModel)
    #endif
  ...
  @objc func onCallingUIKitPressed() {
    #if DEBUG
      let uiKitDemoViewController = CallingDemoViewController(
        envConfigSubject: envConfigSubject,
        callingViewModel: callingViewModel,
