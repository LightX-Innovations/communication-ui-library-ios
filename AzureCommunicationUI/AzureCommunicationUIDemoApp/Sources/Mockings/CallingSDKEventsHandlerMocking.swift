...
import Foundation
  ...
  class CallingSDKEventsHandlerMocking: CallingSDKEventsHandler {
    ...
    func addParticipant(status: ParticipantStatus = .connected) {
      Task { @MainActor [weak self] in
        ...
        let newParticipant = ParticipantInfoModel(
          displayName: participantNameIdentifier,
          isSpeaking: false,
          isMuted: true,
          isRemoteUser: true,
      ...
      Task { @MainActor [weak self] in
        ...
        let connectedParticipants = inLobbyParticipants.map { participantInfoModel in
          ParticipantInfoModel(
            displayName: participantInfoModel.displayName,
    ...
    func emitNetworkQualityDiagnostic(
      _ diagnostic: NetworkQualityCallDiagnostic, value: CallDiagnosticQuality
    ) {
      Task { @MainActor [weak self] in
        guard let self else {
          return
