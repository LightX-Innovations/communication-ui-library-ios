...
def getCurrentVersion():
    local_path = (
        calling_info_plist_path
        if selectedComposite == Composite.CALLING
        else chat_info_plist_path
    )
    if new_Version == "":
        sys.exit(
            "new version is required for this script."
    ...
    if new_Version == oldVersion:
        ...
        print(
            "current version is "
            + oldVersion
            + ", will upgrade it to new version "
            + new_Version
        )
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == "-h":
            sys.exit(
                "Expected parameters: 1. -c or --composite of options of CALLING or CHAT 2. -v or --version"
            )
    ...
    telemetry = (
        calling_telemetry_Tag_path
        if selectedComposite == Composite.CALLING
        else chat_telemetry_Tag_path
    )
    telemetry_pattern = (
        calling_pattern_telemetry
        if selectedComposite == Composite.CALLING
        else chat_pattern_telemetry
    )
    update(
        acs_UI_library_Path + telemetry,
    ...
    main_repo = (
        calling_main_readme
        if selectedComposite == Composite.CALLING
        else chat_main_readme
    )
    update(acs_UI_library_Path + main_repo, oldVersion, new_Version)
    ...
    pbx_path = (
        calling_pbx_path if selectedComposite == Composite.CALLING else chat_pbx_path
    )
    update(
        acs_UI_library_Path + pbx_path,
