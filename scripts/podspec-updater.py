...
# chat
chat_podspec = (
    "AzureCommunicationUI/sdk/AzureCommunicationUIChat/AzureCommunicationUIChat.podspec"
)
...
def update_version():
    local_path = (
        calling_podspec if selectedComposite == Composite.CALLING else chat_podspec
    )
    ...
    print(
        "Step 1 of 3, spec.version updated to "
        + new_version
        + " from "
        + current_version
    )
...
def update_swift():
    ...
    print(
        "Step 2 of 3, spec.swift_version updated to "
        + new_version
        + " from "
        + current_version
    )
...
def update_platform():
    ...
    update(acs_UI_library_Path + local_path_pod, current_version, new_version)
    print(
        "Step 3 of 3, spec.platform updated to "
        + new_version
        + " from "
        + current_version
    )
...
def main(argv):
    try:
        opts, args = getopt.getopt(
            argv, "hv:c:s:p:", ["help", "version=", "composite=", "swift=", "platform="]
        )
    ...
    if selectedComposite == Composite.UNKNOWN:
        sys.exit(
            "Error: Composite is Unknown. Supported Composites are 'calling' and 'chat'."
        )
    if swift_version == "":
        print("Warning: No swift version given. Swift version update would be skipped.")
    if platform_version == "":
        print(
            "Warning: No minimum platform version given. Platform update would be skipped. "
        )
    update_version()
    update_swift()
    update_platform()
